...
Dans ce projet, nous nous sommes intéressés uniquement aux algorithmes de génération des \textit{soft shadows} car ils permettent également de générer des \textit{hard shadows}.
...
\item{Nous avons utilisé pyQt comme bibliothèque qui s'interfacera avec la célèbre bibliothèque Qt 5. Nous avons utilisé cette interface graphique étant donnée sa facilité d'utilisation et son caractère complet. De plus nous avions pour la plupart déjà utilisée cette dernière durant notre cursus.}
...
Plusieurs scènes ont été créées pour présenter et mettre en application les différents algorithmes. 
...
Pour cela nous avons utilisé deux ressources :
...
Les modèle 3D au format OBJ proviennent de sites internet proposant des modèles libres de droit et sont très divers de manière à pouvoir réaliser beaucoup de tests différents : vélo, fractal de menger, mario et luigi, arbres, ...
...
Une première qui consiste à faire tourner de manière circulaire la caméra autour du centre de la scène.

une deuxième qui consiste à faire tourner la lampe sélectionnée dans la barre d'outils.

\subsection{Metrics utilisés}

Pour mesurer la performance d'un algorithme nous avons utilisé différentes unités de mesure.
...
On peut également ajouter du réalisme en modifiant la répartition du disque de Poisson de manière à flouter les contours mais en diminuant trop la répartition, on voit apparaître des nuages de points dans l'ombre. Quand on augmente trop la répartition, le floutage dispara\^it.
Cet algorithme doit donc \^etre adapté au type de scène (objets sphériques, parallélipipèdiques,...) et de lumière.
...
Le plus performant mais également le moins précis des trois algorithmes d'ombres que nous avons ici considéré est le shadow mapping. Celui-ci est toutefois le seul a offrir plusieurs niveaux de précision en fonction de la résolution qui est choisie pour la shadow map.
