...
Nous n'avons implémenté que des lumières ponctuelles de couleur blanche. Il est par contre possible d'en mettre autant que souhaité et de changer leur position individuellement. De plus il est possible via un sélectionneur dans la barre d'outils de les déplacer en temps réel. Cela permet de tester les positions critiques des ombres ou des glitch pourrait apparaitre.
...
\begin{figure}[H]
\includegraphics[scale=0.37]{images/shadow_volume.png}
\centering
...
Ceci permet d'obtenir un nombre de \textit{fps} inférieur au shadow mapping mais bien suffisant pour que l'image soit fluide.
...
On peut également ajouter du réalisme en modifiant la répartition du disque de Poisson de manière à flouter les contours mais en diminuant trop la répartition, on voit apparaître des nuages de points dans l'ombre. Quand on augmente trop la répartition, le floutage disparai\^t.
Cet algorithme doit donc e\^tre adapté au type de scène (objets sphériques, parallélipipèdiques,...) et de lumière.

\subsection{Ray tracing}

Les ombres obtenues par cette algorithme sont très réalistes, étant donné le fait qu'il considère chaque pixel de la scène indépendemment et qu'OpenGL effectue automatiquement une interpolation des pixels pour réduire le crénelage. L'ombre est donc appliquée sur chaque pixel un à un, et cet algorithme offre donc une précision au pixel près. Le ray tracing permet un excellent rendu d'ombre mais on ne peut pas en mesurer correctement les performances car il ne s'agit pas à proprement parler d'une scène interactive. Pour implémenter du ray tracing interactivement, il faudrait un matériel très performant.

\todo{qqun sait rajouter qque chose ici ?}
...
\todo{Aucun ombre vs le reste}

\todo{Auto ombre vs le reste}
...
La technique du ray tracing peut reproduit de manières exactes des phénomènes physique et optique. Il est donc beaucoup utilisé pour ce qui nécessite un rendu de haute qualité comme par exemple pour l'édition d'image du logiciel Adobe Photoshop. Le nom de leur logiciel est Adobe Ray Tracer.
